...
var stockGrowthSector = map[string]string{
	...
	"AMD":  "AI_TECH", "INTC": "AI_TECH", "TSM": "AI_TECH",
	"TSLA": "CLEAN_ENERGY",
...
var stockInsights = map[string]stockInsight{
	...
	"TSM": {
		thesis: []string{
			"Sole 3nm/2nm chip foundry on Earth — NVIDIA, AMD, Apple, Qualcomm all 100% dependent on TSMC",
			"CoWoS advanced-packaging capacity (the bottleneck for every AI GPU) growing 60%+ YoY through 2027",
			"Arizona, Japan, Germany fabs derisking the Taiwan-concentration risk — diversified by 2026–27",
			"Operating margin 47%+ — best in semiconductor industry, structurally protected by cost moat",
			"AI accelerators 60%+ of revenue mix by 2026 — fastest-growing, highest-margin segment",
			"Forward P/E ~22x (vs NVDA 40x, ASML 35x) — cheapest AI infrastructure pick by far",
			"2nm process volume in late-2025 — 2–3 year lead over Samsung Foundry / Intel 18A",
		},
		risks: []string{
			"Geopolitical: Taiwan-China tensions remain the dominant tail risk for long holders",
			"AI capex cycle peak — any pause in hyperscaler GPU buying directly impacts foundry orders",
			"Intel 18A and Samsung GAA-FET could pressure pricing power if they ship at scale by 2027",
			"USD/TWD strength compresses margins; ~70% revenue is USD denominated",
			"ADR (TSM) trades at 15–20% discount to TPE:2330 due to Taiwan settlement/withholding tax",
		},
	},
	"META": {
		thesis: []string{
...
func (a *LongTermUSAnalyzer) analyzePick(
	...
	pe, fpE, pb, epsG, revG, roe, roa, de, pm, divY = applyADRFallback(stock.Symbol, pe, fpE, pb, epsG, revG, roe, roa, de, pm, divY)
...
// adrFallbackFunds holds known fundamentals for ADRs / stocks where Yahoo
// summary endpoint doesn't return data. Values reviewed Q1 2026.
// Fields: PE, ForwardPE, P/B, EPSGrowth%, RevGrowth%, ROE%, ROA%, D/E, ProfitMargin%, DivYield%
var adrFallbackFunds = map[string][10]float64{
	// Taiwan Semiconductor Manufacturing — ADR
	"TSM": {26.0, 22.0, 8.5, 36.0, 30.0, 28.0, 16.0, 0.25, 40.0, 1.4},
	// Novo Nordisk — ADR
	"NVO": {28.0, 24.0, 18.0, 32.0, 25.0, 84.0, 32.0, 0.20, 35.0, 1.1},
}

// applyADRFallback fills in known fundamentals for stocks Yahoo doesn't cover.
// Only overrides zero/missing values — real data always wins.
func applyADRFallback(symbol string, pe, fpE, pb, epsG, revG, roe, roa, de, pm, divY float64) (
	float64, float64, float64, float64, float64, float64, float64, float64, float64, float64) {
	fb, ok := adrFallbackFunds[symbol]
	if !ok {
		return pe, fpE, pb, epsG, revG, roe, roa, de, pm, divY
	}
	if pe == 0    { pe    = fb[0] }
	if fpE == 0   { fpE   = fb[1] }
	if pb == 0    { pb    = fb[2] }
